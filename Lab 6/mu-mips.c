...
uint32_t cache_reads(uint32_t addr)
{
	...
	printf("Index in reads is %x\n", index);
	uint32_t tag = (addr & 0xFFFFFF00) >> 8;
	//uint32_t tag = (addr >> 30) & ((1u << 2) -1);
	printf("tag in writes is %x\n", tag);
	//uint32_t tag = EX_MEM.ALUOutput >> 8;
	uint32_t offsetW = ((addr & 0x000000FF) >> 2) & 0b11;
	printf("Offset in reads is: %x\n", offsetW); 

	//if tags don't match or not valid e then miss
	if (L1Cache.blocks[index].tag != tag || L1Cache.blocks[index].valid != 1)
	{
		L1Cache.blocks[index].tag = tag; //check tag
		//reading each word for the block into cache
		switch(offsetW) {
			case 0x00: {
				printf("made it here\n");
			...
			case 0x01: {
				...
				L1Cache.blocks[index].words[1] = mem_read_32((addr & 0xFFFFFFF0) + (0<<2));
				break;
			}
			case 0x02:{
				printf("made it here 2\n");
				L1Cache.blocks[index].words[2] = mem_read_32((addr & 0xFFFFFFF0) + (1<<2));
				break;
			}
			case 0x03: {
				printf("made it here 3\n");
				L1Cache.blocks[index].words[3] = mem_read_32((addr & 0xFFFFFFF0) + (3<<2));
				break;
			}
			default: 
				printf("went to default");
		}	
		//cache miss so 100 cycles
		CACHE_MISS_FLAG = 1;
		cache_misses++;
	}
	else
		cache_hits++; //cache hit no probs

	return L1Cache.blocks[index].words[offsetW]; //return the needed cache
}
...
void cache_writes(uint32_t addr, uint32_t new)
{
	...
	switch (opcode) // store instruction if not in cache
	{
		case 0x28: //SB
	...
	L1Cache.blocks[index].words[offsetW] = data; //writing data from store instruciton to the cache blocks
